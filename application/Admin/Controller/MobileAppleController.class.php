...
class MobileAppleController extends AdminbaseController{
	...
	*/
	...
	protected function uploadsexl($config){
		...
		//}else{// 上传成功 
			...
			if($data){
				S("addMA".$str.session(ADMIN_ID),null);
				S("addMA".$str.session(ADMIN_ID),$data);
				S("addexta",$info["file"]["ext"]);
			}
	...
	public function addmobile(){
		if(!S("addMAxls".session(ADMIN_ID))){
			$entry['status']=3;
		...
		$str="xls";
		$dataary=S("addMA".$str.session(ADMIN_ID));
		$data=$dataary['data'];
		if($data){
		...
		$dataary['data']=$data;
		S("addMA".$str.session(ADMIN_ID),$dataary);
	...
	public function addmobile100(){
		...
		if(!S("addextaxls".session(ADMIN_ID))){
			$entry['status']=3;
			$this->ajaxreturn($entry);
			exit();
		}
	...
	public function addappletxt(){
		...
		$dataary=S("addMA".session(ADMIN_ID));
		...
		$dataary=$data;
		S("addMA".session(ADMIN_ID),$dataary);
